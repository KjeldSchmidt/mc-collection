...
// class MarioKart : public ColorMode {
// public:
// 	uint16_t Update(CRGB* leds_out) override
// 	{
//
// 	}
//
...
public:
	explicit ColorPulse() : startMillis(millis()) {}

	uint16 Update( CRGB *leds_out ) override {
		uint64_t currentMillis = millis();
		if (currentMillis - startMillis >= 10650)
		{
			for ( uint8_t i = 0; i < NUM_LEDS; i++ )
			{
...
private:
	...
	static double calc_start_hue(uint64_t currentMillis) {
		double wave1 = sin(double(currentMillis) / (135.00));
		double wave2 = sin(double(currentMillis) / (265.00));
		double wave3 = sin(double(currentMillis) / (55.00));
		return 511 * (wave1*wave2*wave3) + 255;
	}

	static double calc_end_hue(uint64_t currentMillis) {
		double wave1 = sin(double(currentMillis) / (127.00));
		double wave2 = sin(double(currentMillis) / (256.50));
		double wave3 = sin(double(currentMillis) / (65.50));
		return 511 * (wave1*wave2*wave3) + 255;
	}
};
