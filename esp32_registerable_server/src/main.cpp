...
/**
 * This should illustrate the ability for an ESP32 to present itself as a wifi server if it fails
 * to create a usable WiFi connection. This will serve a page that can be used to set an SSID and
 * password for another connection, thus registering the device to a network without needing
 * to reupload new code.
 *
 * Uses https://github.com/Hieromon/AutoConnect as the core component
 */
